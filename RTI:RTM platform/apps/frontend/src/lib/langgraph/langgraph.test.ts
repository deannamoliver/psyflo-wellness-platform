...
// biome-ignore lint/correctness/noUnusedVariables: Interface is used in function signatures
interface Scenario {
  scenario_id?: string;
  persona: string;
  situation: string;
...
// biome-ignore lint/suspicious/noExplicitAny: Test utility handles dynamic message formats
...
class ChatGoogleGenerativeAI implements LLMClient {
  // biome-ignore lint/suspicious/noExplicitAny: Google Generative AI client type
  private client: any;
  ...
  async invoke(input: string | { messages?: Message[] }): Promise<LLMResponse> {
    try {
      // biome-ignore lint/suspicious/noExplicitAny: LLM response type varies
      let response: any;
      if (typeof input === "string") {
        response = await this.client.invoke(input);
      } else if (input.messages) {
        // Convert simple Message objects to LangChain's BaseMessage instances
        const {
          AIMessage,
          HumanMessage,
          SystemMessage,
        ...
        // Some LangChain clients accept the raw array, others expect an object
        // with `messages`. Try both shapes so the wrapper is resilient.
        try {
          response = await this.client.invoke(langChainMessages);
        } catch (_err) {
          response = await this.client.invoke({ messages: langChainMessages });
        }
      } else {
      ...
      return { content };
      // biome-ignore lint/suspicious/noExplicitAny: Error type for catch block
    } catch (err: any) {
...
const USER_SIMULATOR_MODEL =
  process.env["USER_SIMULATOR_MODEL"] || "gemini-2.0-flash";
const EVALUATOR_MODEL = process.env["EVALUATOR_MODEL"] || "gemini-2.5-pro";
const CLASSIFIER_MODEL = process.env["CLASSIFIER_MODEL"] || "gemini-2.0-flash";
...
async function loadScenarios(filePath: string): Promise<Scenario[]> {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: Dynamic CSV parsing
    const obj: any = {};
...
interface Turn {
  ...
  // biome-ignore lint/suspicious/noExplicitAny: Raw graph response type varies
  raw_response?: any;
}

async function runConversation(
  // biome-ignore lint/suspicious/noExplicitAny: LangGraph compiled graph type
  graph: any,
  scenario: Scenario,
): Promise<{
  ...
  const userPersonaPrompt = `You are role-playing as a teenager. Your persona is: ${scenario.persona}. Your current situation is: ${scenario.situation}. Your responses should be brief, in character, and reflect the language of a teenager. Do not solve the problem yourself; let the chatbot guide you.`;
...
async function isConversationOver(
  ...
  - "CONTINUE_CONVERSATION": The user is still engaged, asking questions, or giving neutral responses.
  
  Does the chatbot seem to be trying to end the conversation? (e.g., saying goodbye, "take care", asking "anything else I can help with?").
  
  Based on this, should the conversation end?
  - If the chatbot tries to end AND the user's intent is "END_CONVERSATION", respond "yes".
  - If the user's intent is "END_CONVERSATION" and the chatbot's response agrees or also tries to end, respond "yes".
  - If the user's intent is "DISMISSIVE_WITHDRAWAL", the conversation should almost never end. Respond "no".
...
async function evaluateConversation(
  transcript: string,
  rubric: string,
  // biome-ignore lint/suspicious/noExplicitAny: Evaluation result structure varies
): Promise<any> {
  ...
  // biome-ignore lint/suspicious/noExplicitAny: Normalized evaluation keys
...
function _formatTranscript(
  ...
  if (scenario) {
    ...
    transcript += `Situation: ${scenario.situation}\n`;
    transcript += "----------------------\n\n";
    transcript +=
      "################################################################\n\n";
  }
...
function formatTranscriptFromTurns(turns: Turn[], scenario?: Scenario): string {
  ...
  for (let i = 0; i < turns.length; i++) {
    const t = turns[i];
    // biome-ignore lint/suspicious/noExplicitAny: Raw response type varies
...
async function appendResultToCsv(
  // biome-ignore lint/suspicious/noExplicitAny: Dynamic result structure
  ...
  // The CSV header is now written by the main test runner function.

  const row = `${headers.map((h) => escapeCsv(String(result[h] ?? ""))).join(",")}\n`;
...
async function runSingleScenario(
  ...
  return retryWithBackoff(async () => {
    // eslint-disable-next-line no-console
    console.log(
      `--- Running Scenario ID: ${scenario.scenario_id}, Situation: ${scenario.situation} ---`,
    );
    ...
    await fs.writeFile(
      transcriptFilename,
      transcript + terminationReasonText + evaluationText,
      { encoding: "utf-8" },
    );
    ...
    // biome-ignore lint/suspicious/noExplicitAny: Dynamic result data structure
...
describe("Chatbot Evaluation Runner", () => {
  test("should run the full evaluation process for scenarios_fringe_cases", async () => {
