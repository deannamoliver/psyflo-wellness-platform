...
const OutOfBoundsDay = ({ day }: OutOfBoundsDayProps) => (
  <div className="relative h-24 w-full bg-secondary p-1 text-center text-md text-muted-foreground">
    {day}
  </div>
);
...
export const CalendarBody = ({ features, children }: CalendarBodyProps) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      <div
        className="relative flex h-24 w-full flex-col gap-1 p-1 text-center text-md text-muted-foreground"
        key={day}
      >
        {day}
        <div>
  ...
  return (
    <div className="grid flex-grow grid-cols-7">
      {days.map((day, index) => (
        <div
          className={cn(
            "border-border/50 border-r p-2 text-right font-medium text-muted-foreground text-xs",
          )}
...
export const CalendarHeader = ({ className }: CalendarHeaderProps) => {
  ...
  return (
    <div className={cn("grid flex-grow grid-cols-7", className)}>
      {daysData.map((day) => (
        <div
          className="p-3 text-center text-md text-muted-foreground"
          key={day}
        >
          {day}
        </div>
