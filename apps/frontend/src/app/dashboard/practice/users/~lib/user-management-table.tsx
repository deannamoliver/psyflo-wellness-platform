...
export function UserManagementTable({ users }: { users: UserRow[] }) {
  ...
  const [roleFilter, setRoleFilter] = useState<"all" | "admin" | "therapist" | "wellness_coach">("all");
  ...
  if (roleFilter !== "all") {
    ...
    filtered = filtered.filter((u) => u.role === roleMap[roleFilter]);
  }
      ...
      <div className="flex items-center justify-between border-b px-5 py-4">
        <div className="flex items-center gap-3">
          <h3 className="text-sm font-semibold text-gray-900">
            Practice Users <span className="ml-1 text-gray-500">({users.length})</span>
          </h3>
          <button
            type="button"
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5">
            {(["all", "admin", "therapist", "wellness_coach"] as const).map((r) => (
              <button
                key={r}
                type="button"
                onClick={() => {
              ...
              >
                {r === "all" ? "All" : r === "admin" ? "Admin" : r === "therapist" ? "Therapist" : "Coach"}
              </button>
            ))}
          </div>
            ...
            {paginated.length === 0 ? (
              ...
              paginated.map((user) => (
                <tr key={user.id} className="transition-colors hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3">
                    <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium", getRoleBadgeColor(user.role))}>
                      {user.role}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium capitalize", getStatusBadgeColor(user.status))}>
                      {user.status}
                    </span>
                  </td>
