...
export default async function StudentOverviewPage({
  ...
  // ── Fetch conversation count for adherence ────────────────────────────────────────────────────
  const convoCount = await db.admin.select({ count: count() }).from(chatSessions).where(eq(chatSessions.userId, studentId)).then((r) => r[0]?.count ?? 0);
