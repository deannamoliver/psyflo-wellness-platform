...
async function ActiveSafetyAlertsCard({ schoolId }: { schoolId: string }) {
...
async function TotalStudentsCard({ schoolId }: { schoolId: string }) {
  const db = await serverDrizzle();
  const result = await db.admin
    .select({ count: count() })
    .from(profiles)
...
export async function CoachStatsOverview({ schoolId }: { schoolId: string }) {
  return (
    <div className={SUMMARY_CARDS_GRID_CLASS}>
      <Suspense fallback={<StatCardSkeleton />}>
        <ActiveSafetyAlertsCard schoolId={schoolId} />
      </Suspense>
      <Suspense fallback={<StatCardSkeleton />}>
        <TotalStudentsCard schoolId={schoolId} />
      </Suspense>
      <Suspense fallback={<StatCardSkeleton />}>
