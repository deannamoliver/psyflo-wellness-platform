...
export type TreatmentPlan = {
  ...
  notes: string;
  activities: Activity[];
  // Extended fields for generated plans
  diagnosisCode?: string;
  diagnosisLabel?: string;
  richGoals?: TreatmentPlanGoal[];
...
export function buildRTMPatients(students: RealStudent[]): RTMPatient[] {
  return students.map((student) => {
    ...
    const studentName = student?.name ?? "Patient";
    const overlay = RTM_OVERLAYS[studentName] ?? {
      ...DEFAULT_OVERLAY,
      dataCollectionDays: (hashCode(studentId) % 20) + 5,
      clinicianMinutesLogged: (hashCode(studentId + "m") % 30) + 5,
      interactiveCommunications: (hashCode(studentId + "c") % 3) + 1,
    };
    return {
      id: studentId,
      name: studentName,
