...
import { saveSoliCustomization } from "./action";
...
export default function CustomizePage() {
  ...
  const handleContinue = async () => {
    ...
    // Save to database
    try {
      await saveSoliCustomization(selectedColor, selectedShape);
      // Navigate on success
      router.push("/welcome/expect");
    } catch (error) {
      // Show error and allow retry
      setIsSaving(false);
      toast.error(
        "There was an issue saving your customization. Please try again.",
      );
      console.error("Failed to save soli customization to database:", error);
    }
  };
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      ...
      <main className="flex flex-1 flex-col items-center px-4 pt-8 md:justify-center md:px-8 md:pt-0">
        {/* Create Badge with decorative sparkle */}
        ...
        <div className="relative mb-2 flex items-center justify-center md:mb-4">
          <Image
            ...
            width={59}
            height={68}
            className="-left-10 md:-left-13 2xl:-left-17 absolute top-6 h-10 w-9 md:top-14 md:h-13 md:w-11 2xl:top-20 2xl:h-17 2xl:w-15"
          />
          <h1 className="text-center font-bold text-3xl text-gray-900 md:text-6xl 2xl:text-8xl">
            Make me yours
          </h1>
        </div>

        {/* Subtext */}
        <div className="mb-8 flex items-center justify-center md:mb-10 2xl:mb-16">
          <p className="text-center text-base text-gray-600 md:text-xl 2xl:text-[30px]">
            Customize how I look!
          </p>
        </div>

        {/* Customization Area */}
        <div className="mb-8 flex w-full max-w-md flex-col items-center gap-8 md:mb-10 md:max-w-none md:flex-row md:items-start md:justify-center md:gap-12 2xl:mb-16 2xl:gap-16">
          {/* Mascot Preview Card */}
          <div className="flex flex-col items-center rounded-3xl border-[0.5px] border-gray-100 bg-white px-8 py-6 shadow-[0_20px_40px_-5px_rgba(210,233,255,1)] md:px-12 md:py-8">
            <div className="relative mb-2 h-36 w-36 md:mb-4 md:h-40 md:w-40 2xl:h-52 2xl:w-52">
              <Image
                src={mascotImage}
                alt="Soli mascot"
                fill
            ...
            <p className="font-semibold text-gray-900 text-xl md:text-xl 2xl:text-2xl">
              Soli
            </p>
          </div>
              ...
              <div className="flex flex-wrap justify-start gap-2 md:gap-3">
                {COLORS.map((color) => (
                  <button
                    ...
                    className="relative h-9 w-9 rounded-full transition-transform hover:scale-105 md:h-10 md:w-10 2xl:h-12 2xl:w-12"
              ...
              <div className="grid grid-cols-4 gap-2 md:flex md:gap-3">
                {SHAPES.map((shape) => (
                  <button
                    ...
                    className={`flex h-24 w-20 flex-col items-center justify-center gap-1.5 rounded-2xl border-2 transition-colors md:h-28 md:w-24 md:gap-2 2xl:h-32 2xl:w-28 ${
        ...
        <Button
          ...
          disabled={isSaving}
          className="rounded-full px-8 py-6 text-base hover:bg-[#C8CCD5] md:px-[20px] md:py-6 md:text-lg 2xl:py-7 2xl:text-xl"
          size="lg"
        >
          {isSaving ? (
            <>
